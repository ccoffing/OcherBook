...
int RendererCdk::outputWrapped(clc::Buffer* b, unsigned int strOffset, bool doBlit)
{
...
{
	clc::Log::info("ocher.render.cdk", "render page %u %u", pageNum, doBlit);
	m_x = 0;
	m_y = 0;
	if (m_height) {
		wclear(m_scr);
	...
	if (!pageNum) {
		...
		clc::Log::error("ocher.renderer.cdk", "page %u not found", pageNum);
		return -1;
	}
	...
	for (unsigned int i = layoutOffset; i < N; ) {
		...
		switch (opType) {
			case Layout::OpPushTextAttr:
				clc::Log::debug("ocher.render.cdk", "OpPushTextAttr");
				switch (op) {
					...
					case Layout::AttrSizeAbs:
						...
						break;
					default:
						clc::Log::error("ocher.render.cdk", "unknown OpPushTextAttr");
						ASSERT(0);
						break;
				}
				break;
			case Layout::OpPushLineAttr:
				clc::Log::debug("ocher.render.cdk", "OpPushLineAttr");
				switch (op) {
					...
					case Layout::LineJustifyRight:
						break;
					default:
						clc::Log::error("ocher.render.cdk", "unknown OpPushLineAttr");
						ASSERT(0);
						break;
				}
				break;
			case Layout::OpCmd:
				switch (op) {
					case Layout::CmdPopAttr:
						clc::Log::trace("ocher.render.cdk", "OpCmd CmdPopAttr");
						if (arg == 0)
							arg = 1;
						while (arg--)
							popAttrs();
					...
					case Layout::CmdOutputStr: {
						clc::Log::trace("ocher.render.cdk", "OpCmd CmdOutputStr");
						...
						if (breakOffset >= 0) {
							...
							clc::Log::debug("ocher.renderer.cdk", "page %u break", pageNum);
							return 0;
						}
						i += sizeof(clc::Buffer*);
						break;
					...
					case Layout::CmdForcePage:
						clc::Log::trace("ocher.render.cdk", "OpCmd CmdForcePage");
						break;
					default:
						clc::Log::error("ocher.render.cdk", "unknown OpCmd");
						ASSERT(0);
						break;
				}
				break;
			...
			case Layout::OpImage:
				break;
			default:
				clc::Log::error("ocher.render.cdk", "unknown op type");
				ASSERT(0);
				break;

		};
	}
	clc::Log::debug("ocher.renderer.cdk", "page %u done", pageNum);
	refreshCDKScreen(m_screen);
	return 1;
}
